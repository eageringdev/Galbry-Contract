...
contract Marketplace is ReentrancyGuard {
        ...
        )
    {
        ...
        return (totalItemCount, actualEnd, marketItems);
    }
    ...
    /**
     * @dev Fetch custom market items
     * return total, fromIndex, items
     */
    function fetchAvailableMarketItemsCustom(uint256 from, uint256 count)
        public
        view
        returns (
            uint256,
        ...
        )
    {
        ...
        uint256 returnFromIndex = i;
        if (actualCount == 0) {
            return (itemsCount, returnFromIndex, new MarketItem[](0));
        }
        MarketItem[] memory marketItems = new MarketItem[](actualCount);

        uint256 currentIndex = 0;
        for (i = from; i < returnFromIndex; i++) {
            // Is this refactor better than the original implementation?
            // https://github.com/dabit3/polygon-ethereum-nextjs-marketplace/blob/main/contracts/Market.sol#L111
            // If so, is it better to use memory or storage here?
            MarketItem memory item = marketItemIdToMarketItem[i + 1];
        ...
        return (itemsCount, returnFromIndex, marketItems);
    }
